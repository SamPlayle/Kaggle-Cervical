...
trainingdf <- cbind(training, trainratios)
testingdf <- cbind(testing, testratios)
...
logloss <- -sum(
  log(probs4$"1"*(testing$type==1) 
       + probs4$"2"*(testing$type==2) 
       + probs4$"3"*(testing$type==3)
       )
  )/nrow(probs4)
  ...
  unknowndf <- cbind(unknown, unknownratios)
  
  
  probsunknown <- predict(model4, 
                          newdata = unknowndf, 
                          type = "prob"
  )
